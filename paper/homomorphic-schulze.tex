...
(* Rearranging it. Giving high level details here and more 
 ...
 Overall, the cryptographic primitives, which are literal translation of 
 functionality described above to preserve the privacy
...
then there is corresponding primitive for verifying it. 
The verification primitive 
takes zero-knowledge-proof with some other public parameters 
...
otherwise not. At this point, the natural question is 
...
we have formally verified the outcome of (encrypted)ballots homomorphic
counting against 
a fully verified (plaintext)ballots counting\cite{Pattinson:2017:SVE}
assuming that the (encrypted)ballots decrypt to the (plaintext)ballots, 
and all the zero-knowledge-proofs are constructed honestly.
...
Now we explain the  cryptographic primitives as described above 
in detail. In order to encrypt a value, and decrypt a value with 
...
honest decryption zero-knowledge-proof. Technically, we don't need 
...
is verifiable. We stand on shoulder of Pedersen's 
...
no adversary can guess it from commitment (hiding), 
and we can't change it in future 
...
A plaintext is represented as Coq Integer, ciphertext is abstract type, 
 ...
 plaintext ballot, denoted by $pballot$,  is square ballot of plaintext,
 and axiom  $decryption\_left\_inverse$ states that if we encrypt 
 a plaintext $pt$ with $publickey$ (encapsulated in $group$ which is 
 ...
 so we point our reader to have a look at (* some reference goes here *)
...
formalization, plaintext is represented as concrete data type (Coq Integer),
 ...
 two fold:
 $i)$ evidencing the validity of ballot which requires inspecting 
 the values of decrypted shuffled ballot inside theorem prover, 
 and $ii)$ 
 
 we decrypted the 
 fully constructed margin matrix on which we run a formally verified 
 computation and compute winners.  
