...
\begin{itemize}
 ...
 a notion of linear preference ordering for all candidates, and 
 we prove that we can decide for each ballot if it valid or not 
 \begin{verbatim}
 ...
 By now, any attentive reader would note that we have claimed that we don't decrypt
 the ballot to preserve the ballot privacy, so how can we decide the validity 
 without decrypting the original ballot. 
\end{itemize}
