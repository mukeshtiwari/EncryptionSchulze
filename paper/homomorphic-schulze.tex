...
knowledge proof, and verification of zero knowledge proof.  



We describe homomorphic Schulze counting as dependent inductive data type,
\textbf{ECount}. ECount is parametrised by, group, used 
...
\item Binding: A dishonest party cannot open his commitment in more  
	 	than one way
\item Non-correlation:  A dishonest party cannot commit to a value that is in 
	some significant way correlated to the honest party's value.
\end{itemize}

We hide the permutation, \textbf{pi}, by generating the 
commitment, \textbf{cpi}, and publishing it, and generate zero knowledge proof
, \textbf{zkpcpi}, using permutation pi and its commitment cpi 
which can be used later to prove in zero knowledge 
that ballot v is permutation of  ballot u by (secret) permutation pi 
without revealing pi. In 
order to show that b is honest decryption of ballot w we construct 
honest decryption zero knowledge proof \textbf{zkpdec}.
...
captures each step of execution which can be produced as evidence, 
known as certificate, to scrutineers for inspection.
...
with corresponding primitives in Unicrypt library. Since the Unicrypt library is 
