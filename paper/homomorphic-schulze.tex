...
\newline\textbf{Decryption Primitive:}
\begin{lstlisting}[frame=single,basicstyle=\ttfamily\footnotesize]
Variable decrypt_message : 
    Group -> Prikey -> ciphertext ->  plaintext.
\end{lstlisting}
\textbf{ZKP Construction Primitive for honest decryption:}
\begin{lstlisting}[frame=single,basicstyle=\ttfamily\footnotesize]
Variable construct_zero_knowledge_decryption_proof :
    Group -> Prikey -> ciphertext -> DecZkp.
\end{lstlisting}
\pagebreak\textbf{ZKP Verification Primitive for honest decryption:}
\begin{lstlisting}[frame=single,basicstyle=\ttfamily\footnotesize]
...
Some comments are in order.  We represent plain text by the Coq data
type of integers (as this is the information that is to be
encrypted), but leave ciphertext as an abstract type, to be
instantiated later.  

{\tt DP: some detail about verification / generation primitives.
Possibly honest decryption, as it is easier?}
...
\begin{description}
  ...
  \item[correctness condition] here, the constuctor may only be applied if
...
Schematically, we have the following representation with
data in first column, and 
assertions about the data in second column.
\begin{mdframed}[]
Ecax
\begin{center}
...
\end{lstlisting}
\footnote{DP: the primitives displayed here (verify zkp) should be
amongst those that are explained in code also earlier}
...
Schematically, we have  the following, again with the data on the
left, and the correctness conditions on the right.
...
The first theorem, \texttt{insert name here}, reproduced below shows
...
Theorem \texttt{insert name here} establishes the opposite.

\begin{verbatim}
  Lemma final_correctness (group : Group) (bs : list ballot): 
    forall (pbs : list pballot) (ebs : list eballot) (w : cand -> bool),
      (pbs = map (fun x => (fun c d => decrypt_message grp privatekey (x c d))) ebs) ->
      (mapping_ballot_pballot bs pbs) ->
    Count bs (winners w) -> ECount group ebs (ewinners w).
      
  Lemma final_correctness_rev (grp : Group) (bs : list ballot):
    forall (pbs : list pballot) (ebs : list eballot) (w : cand -> bool)
      (pbs = map (fun x => (fun c d => decrypt_message grp privatekey (x c d))) ebs) ->
      (mapping_ballot_pballot bs pbs) ->
...
The proof, and indeed the formulation, relies on an indictive data
...
show that the (decrypted) final margins agree.
...
state data, verification data, and correction constraints, has been
...
help of the Unicrypt library. 
\footnote{https://github.com/bfh-evg/unicrypt} \footnote{DP: turn
into a proper reference}
...
% 
 ...
 columns of the (row permuted) ballot. Finally, texttt{D} is the
 ...
 plaintext version of the protocol \cite{Pattinson:2007:VSE}.
