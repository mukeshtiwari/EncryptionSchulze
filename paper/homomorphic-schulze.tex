...
In general, every election has purpose to elect candidates based on cast
  ...
  time it opens the possibility of coercion \cite{Benaloh:2009:SSC}, and if we don't publish then it certainly offers the more privacy, but hampers universal 
  ...
  In previous work \cite{} we have demonstrated 
...
The implementation of homomorphic tallying that we are about to
describe follows the same two phases as the protocol: we first
compute the margin matrix, and then compute generalised margins. 
\begin{itemize}
