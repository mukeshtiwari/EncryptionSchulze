...
\begin{itemize}
 ...
 under same public key ( Try to rephrase it!). In short, 
 \begin{center}  
	$enc_{k}(m_{1}) * enc_{k}(m_{2}) = enc_{k} (m_{1} + m_{2})$
 \end{center}
 \item Our choice of representing ballot as matrix comes with price. A malicious 
 client could try to encode all sort of ballots including cyclic ones i.e. candidate
 ...
 not expressed his choices clearly and should not be considered for counting, or 
 he could inflate his favourite candidate by any other number higher that 1. It makes
 the ballot invalid because any value greater that 1 in a ballot would be very similar
 to voter has casted more than one vote. 
