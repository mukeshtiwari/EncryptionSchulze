...
\smallskip\noindent\emph{Cryptographic primitives.} Overall, the
primitives needed to realize the above statements are 
\begin{itemize} 
...
\item generating permutation, generating  Pedersen's 
     ...
     zero-knowledge-proof of Pedersen's commitment, shuffling 
     each row and column of ballot by a given permutation; however, there 
     was no such primitive in library (* I am not convinced about 
     why should we talk about what is in library, but feel free to change it 
      to something more meaningful if it's not making sense *), but 
      it has  primitive for
     (commitment consistent) shuffling a array of ciphertext 
     by a given permutation \cite{Wikstrom:2009:CPS}. We used this primitive 
     to shuffle each row and column of ballot by same permutation, 
