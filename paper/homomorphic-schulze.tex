...
Now that we have explained all the cryptographic primitives, 
we capture the formalization of homomorphic Schulze method using Coq's dependent 
...
\begin{itemize}
 \item The first constructor \textbf{ecax} marks the beginning of election 
       ...
       decrypted margin function \textbf{decm} constructed by cryptographic primitive 
       ...
       \textbf{construct\_zero\_knowledge\_decryption\_proof}
        with
       assertions about validity of data. The constraint, \textbf{us = bs}, states 
...
\item The constructor \textbf{ecvalid} assume that ballot we are going to count 
	  ...
	  v is row permutation u by some (secret) permutation $\pi$ and it is constructed 
	  ...
	  uncounted ballot \textbf{us}, old margin \textbf{m}, and updated margin \textbf{nm} 
	  after counting the ballot u  with usual assertion on data to make sure 
	  the validity of data.
	  
\item The constructor \textbf{ecinvalid} is very similar to to constructor 
 	 \textbf{ecvalid} except it assumes that ballot we are going is not valid, 
 	 so we keep the margin \textbf{m} same and move the invalid ballot \textbf{u}
...
We embedded enough zero 
knowledge proof  data generated from cryptographic primitives in these constructors, 
so that each operation performed can verified independently.  
