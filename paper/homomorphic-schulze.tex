...
entity such as prime number, generator, public key, private key, and 
 functions for performing  encryption, decryption, homomorphic addition,
...
\begin{itemize}
 \item Encrypting a plaintext using public key, and returning ciphertext; decrypting a  
       ciphertext using private key, and returning plaintext;  
       constructing a zero knowledge proof object $zkpdec$ that a plaintext $m$ is honest
       decryption of some ciphertext ($c_{1}$, $c_{2}$); verification of 
       claim that some plaintext $m$ is honest decryption of cipertext 
       ($c_{1}$, $c_{2}$) using zero knowledge proof object $zkpdec$.
 \item Generating a (secret) permutation $\pi$; 
 	   generating commitment of given permutation $\pi$
 	   using Pedersen's commitment scheme $cpi$;
 	   generating zero knowledge proof $zkpcpi$ of some permutation $\pi$ and 
 	   its commitment $cpi$; verification of claim that given 
 	   permutation commitment  $cpi$ is indeed a commitment of some 
 	   permutation $\pi$ using zero knowledge proof $zkpcpi$
 \item Shuffling a ballot $b$ using permutation $\pi$ whose commitment is $cpi$,
 	   and returning shuffled ballot  $b_{\pi}$; 
 	   constructing zero knowledge proof $permzkp$ that 
       ballot $b_{\pi}$ is shuffle of ballot $b$ by permutation $\pi$;
       verifying the claim that ballot $b_{\pi}$ is shuffle of ballot $b$ by some 
       permutation whose commitment is $cpi$ using zero knowledge proof $permzkp$.
\end{itemize}
...
implementation file (Cryptoaxioms.v). One slight deviation from
...
Now we capture the formalization of homomorphic Schulze method using Coq's dependent 
...
The second phase, represented as dependent inductive data type 
...
Now we explain how we embed the data generated by cryptographic primitives in 
constructors of inductive date type \textbf{ECount} during counting of 
ballots to achieve verification. 
\begin{itemize}
 \item The first constructor, \textbf{ecax}, marks the beginning of election 
       by taking list of uncounted ballot \textbf{us}, encrypted margin function 
       \textbf{encm} constructed by cryptographic primitive \textbf{encrypt\_message}
       by encrypting zero matrix, 
       decrypted margin function \textbf{decm} constructed by decrypting 
       encrypted margin function encm using cryptographic primitive 
       \textbf{decrypt\_message}, and zero knowledge proof \textbf{zkpdec} that decm is 
       honest decryption of encm and is constructed by 
       cryptographic primitive 
       \textbf{construct\_zero\_knowledge\_decryption\_proof} \linebreak with
       assertions about validity of data. 
       The constraint, \textbf{us = bs}, states 
       that uncounted ballots are same as 
       cast ballots, every entry in decm is 0, 
       and \textbf{decm} is indeed 
       honest decryption of \textbf{encm} as the honest zero knowledge decryption proof
       verifier function 
       \textbf{verify\_zero\_knowledge\_decryption\_proof} returns true. 
\item The constructor, \textbf{ecvalid}, assumes that ballot we are going to count 
		is valid, takes (Pedersen's) commitment \textbf{cpi} of 
	  (secret) permutation $\pi$ and is generated by cryptographic primitive 
	  \textbf{generatePermutationCommitment},  zero knowledge proof \textbf{zkpcpi} 
	  that permutation commitment cpi is indeed commitment of (secret) 
	  permutation $\pi$, ballot u which we are going to count from uncounted pile,
	   ballot v obtained by shuffling each row 
	  u by (secret) permutation $\pi$ using cryptographic primitive function 
	  \textbf{shuffle} (since shuffle introduces re-encryption, so there is no way 
	  to infer permutation $\pi$ from shuffled ballot v), ballot w is obtained 
	  by permuting each column of ballot v by same permution $\pi$ using the 
	  function \textbf{shuffle}, 
	  ballot b is obtained by decrypting ballot w using function 
	  \textbf{decrypt\_message}, zero knowledge proof \textbf{zkppermuv} that 
	  v is row permutation u by (secret) permutation $\pi$ and it is constructed 
	  via \textbf{shuffle\_zkp}, zero knowledge proof \textbf{zkppermvw} that 
	  w is column permutation of v by same (secret) permutation $\pi$ and it is 
	  also construct by \textbf{shuffle\_zkp}, zero knowledge proof 
	  \textbf{zkpdecw} that b is honest decryption of w and is constructed using 
	  \textbf{construct\_zero\_knowledge\_decryption\_proof}, list of remaining 
	  uncounted ballot \textbf{us}, old margin \textbf{m}, and homomorphically 
	  updated new margin \textbf{nm} 
	  after counting the ballot u  with usual assertion on data to insure
	  the validity of data.
	  
\item The constructor, \textbf{ecinvalid}, is very similar to to constructor 
 	 \textbf{ecvalid} except it assumes that the ballot \textbf{u} we are going 
 	 to count is not valid, 
 	 so we keep the margin, \textbf{m}, unchanged and move the invalid ballot \textbf{u}
 	 into pile of invalid ballots with usual assertions about consistency of data 
 	 embedded in constructor.
 	 
\item The constructor \textbf{ecdecrypt} is marks the finish of 
	  counting ballots, and by this time we have fully constructed the 
	  margin function. This constructor takes list of final invalid ballots 
	  \textbf{inbs}, fully constructed homomorphic margin function \textbf{encm};
	  decrypted margin function \textbf{decm}, and zero knowledge proof \textbf{zkp}
	  of the fact that decrypted margin function decm is honest decryption of 
	  encrypted margin function encm with assertion about validity of data. 
	  
\item The final constructor \textbf{ecfin} takes decrypted margin \textbf{dm}, 
		boolean function \textbf{w} which return true (false) for winners(losers), 
		and \textbf{d} is function that delivers type level evidence of 
		winning or losing with assertion about its consistency with w.
	    
\end{itemize}


We formally state that for every list of ballots, we can find a boolean function 
that decides winners \cite{Pattinson:2017:SVE}, but what more important is that the term 
...
We use Coq extraction mechanism\cite{Letouzey:2003:NEC}  to extract its proofs into
programs retaining all the terms which lives in Type universe, and erases every term 
from Prop universe. We extract our formalized Coq proofs into 
OCaml\cite{Leroy:2013:ORM} program, and instantiate the axiomatic 
cryptographic primitives 
with corresponding primitives in Unicrypt library. Since the library is 
written in Java, and it has not all the functionality inbuilt that we are using, 
e.g. homomorphic addition and honest zero knowledge decryption proof, 
we have written a OCaml wrapper at the top of Unicrypt with the help
Java-OCaml library with extended functionality of homomorphic addition and
 construction of honest decryption zero knowledge proof.  
 
 Once we have connected 
 cryptographic primitives to Unicrypt library, we ran it on set of
 small set of ballots with 3 candidate A, B, and C participating in election.
 In the beginning of computation, we publish 
the information embedded in constructor \textbf{ecax} i.e.  
encrypted zero margin , M, its decryption to show that all the 
entries are 0, and zero knowledge proof of honest decryption. In the next line 
we show the first ballot from counted pile V that we are going to count u, 
piles of invalid ballot so far I, current encrypted margin function M, 
row permuted ballot, v, which is row permutation of ballot, u, we are counting right 
now by 
(secret) permutation, column permutation, w, which is column permutation of 
ballot, v, obtained in previous step by same (secret) permutation, 
decryption of final permuted ballot b, zero know knowledge proof of 
row permutation, zkppermuv, zero knowledge proof of column permutation zkppermvw,
 zero knowledge proof of honest decryption zkpdecw, commitment of secret permutation 
 cpi, zero knowledge proof of cpi is indeed commit of (secret) permutation. 
Depending on the validity of finally permuted ballot, either we update the 
margin function homomorphically, \textbf{ecvalid}, 
or move it to pile of invalid ballots, \textbf{ecinvalid}. We continue 
...
evidence about winning(losing). The numbers appearing below are very large, and 
snipped for brevity.
...
Not only we provide the tally sheet to verify the computation, but we provide 
